/**
 * WorkerTaskMonitor Component
 *
 * Displays worker tasks with real-time status updates.
 * Shows the state of background tasks running for a workspace or spec.
 *
 * Previously named: TaskPanel
 */
...
interface WorkerTaskMonitorProps {
  workspaceId?: string;
  specId?: string;
}
...
function TaskCard({ task, techniqueName }: { task: WorkerTask; techniqueName?: string }) {
  ...
  return (
    <div
      ...
      role="article"
      aria-label={`Task using ${techniqueName || 'Unknown technique'}`}
    >
      <div className="flex justify-between items-center px-3 py-2 bg-muted border-b border-border">
        ...
        <span
          className={cn('px-2 py-0.5 text-xs font-semibold uppercase font-mono text-white', stateStyle)}
          role="status"
      ...
      <div className="p-3 text-sm text-muted-foreground space-y-1">
        ...
        {task.error && (
          <div className="text-destructive text-xs" role="alert">{task.error}</div>
        )}
      </div>
      {task.prUrl && (
        <div className="px-3 py-2 border-t border-border">
          <a
            ...
            className="text-sm text-primary hover:underline focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
          >
            PR #{task.prNumber}
          </a>
        </div>
...
/**
 * WorkerTaskMonitor displays worker tasks with real-time status updates
 */
export function WorkerTaskMonitor({ workspaceId, specId }: WorkerTaskMonitorProps) {
  ...
  if (isLoading && tasks.length === 0) {
    return (
      <div className="flex flex-col gap-3 p-4 bg-card text-center text-muted-foreground py-8" aria-busy="true">
        Loading tasks...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col gap-3 p-4 bg-card text-center text-muted-foreground py-8" role="alert">
  ...
  return (
    <div className="flex flex-col gap-3 p-4 bg-card" aria-label="Worker task monitor">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Worker Tasks</h3>
        <button
          onClick={refetch}
          className="px-2 py-1 bg-transparent border border-border rounded text-muted-foreground hover:bg-muted hover:border-muted-foreground transition-colors focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
...
/** @deprecated Use WorkerTaskMonitor instead */
export const TaskPanel = WorkerTaskMonitor;

export default WorkerTaskMonitor;
