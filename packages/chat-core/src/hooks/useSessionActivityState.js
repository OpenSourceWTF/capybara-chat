...
export function useSessionActivityState(sessionId) {
    ...
    useEffect(() => {
        ...
        const handleActivity = (data) => {
            ...
            if (['complete', 'error', 'timeout'].includes(data.activity.status)) {
                dispatch({ type: 'SET_ACTIVITY', payload: null }); // Clear activity
        ...
        const handleBlocked = (data) => dispatch({ type: 'SET_BLOCKED', payload: data });
        const handleHalted = (data) => dispatch({ type: 'SET_HALTED', payload: data });
        const handleHumanInput = (data) => dispatch({ type: 'SET_HUMAN_REQUEST', payload: data });
        const handleContextReset = (data) => dispatch({ type: 'SET_CONTEXT_RESET', payload: data });
        const handleCost = (data) => dispatch({ type: 'SET_COST', payload: data });
    ...
    // --- Actions ---
    const sendHumanInputResponse = useCallback((response) => {
        if (!sessionId || !state.humanRequest)
            return;
        transport.emit(SOCKET_EVENTS.SESSION_HUMAN_INPUT_RESPONSE, {
            sessionId,
            requestId: state.humanRequest.requestId, // Cast to any as type might be missing it
            response,
        });
...
//# sourceMappingURL=useSessionActivityState.js.map